...
class Spice(SimulationCode):
    ...
    def process_config_options(self, config_opts):
        ...
        if spice_version not in [2, 3]:
            raise ValueError(
                f"spice_version given ({spice_version}) was not valid, must be either 2 or 3."
            )
        self.version = spice_version
        soft_restart, full_restart = config_opts.getboolean(
            "soft_restart"
        ), config_opts.getboolean("full_restart")
        if soft_restart and full_restart:
            raise ValueError(
                "The soft and full reset flags were both set to true, please select only one if "
        ...
        if "time_limit" in config_opts:
            try:
                ...
                raise ValueError(
                    'The "time_limit" optional code config option must be a positive, integer number '
                    "of hours."
                )
            print(
    ...
    def print_config_options(self, config_opts):
        ...
        if "time_limit" in config_opts:
            option_list.append(
                ("Spice time limit", config_opts["time_limit"] + " hr(s)")
            )
    ...
    def get_command_line_args(self, config_opts):
        ...
        time_limit = (
            f'-l {int(config_opts["time_limit"])}'
            if "time_limit" in config_opts
            else None
        )
    ...
    def get_submission_script_body(
        ...
        if (
            "time_limit" not in config_opts
            and machine.max_job_time is not None
            and safe_job_time_fl
        ):
        ...
        if "node_dist_string" in call_params:
            ...
            run_command = " ".join(
                [
                    ...
                    "arbitrary",
                    "-w",
                    f'`{executable_dir / "arbitrary.pl"} {call_params["node_dist_string"]}`',
                ]
            )
        else:
        ...
        if backup_fl:
            postcall_str += (
                f"rsync -azvp --exclude='{self.EXE_COPY_SUBFOLDER}*' --exclude='backup*' "
        ...
        if self.version == 3:
            ...
            for i in range(no_species):
                ...
                if int(species_sect["mpi_rank"]) != -1:
                    raise ValueError(
                        f'Invalid mpi_rank on species {species_sect["name"]}, must be set to -1'
                    )
    ...
    def directory_io(
        self, output_dir, config_opts, dryrun_fl, restart_copy_mode=0, print_fl=True
    ):
        ...
        if not restart_fl:
            ...
            if output_dir.exists() and output_dir.is_dir():
                if print_fl:
                    print(
                        f"WARNING: {output_dir} already exists, searching for next available similar directory \n"
                    )
            ...
            if print_fl:
                print(
                    f"WARNING: Directory {output_dir} doesn't look like a {self.name} simulation output folder.\n"
            ...
            raise ValueError(
                f"Desired directory ({output_dir}) is not a {self.name} directory and therefore not "
                f"restartable.\n"
            )
        else:
