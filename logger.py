...
class Logger(object):
    ...
    def __init__(
        self,
        api_json_filename="client_secret.json",
        gsheet_name="spice-submission-db",
        log_format=None,
    ):
        """
        ...
        if gspread is not None and ServiceAccountCredentials is not None:
            self.creds = ServiceAccountCredentials.from_json_keyfile_name(
                api_json_filename, self.SCOPE
            )
            self.client = gspread.authorize(self.creds)
        else:
            warn("gspread is not available; logging disabled.")
            self.creds = None
            self.client = None
        ...
        # Attempt to set up client connection if possible
        if self.client is not None:
            try:
                self.sheet = self.client.open(gsheet_name).sheet1
            except Exception as e:  # pragma: no cover - connection errors
                print(
                    "Unable to connect to google spreadsheet, logging not possible with current settings. "
                    f"{e}"
                )
                self.sheet = None
        else:
    ...
    def update_log(self, log_data, backup_data=True, dry_run_fl=False):
        index = len(self.sheet.get_all_values()) if self.sheet is not None else 0
        ...
        if self.sheet is not None and not dry_run_fl:
            ...
            print(
                f"{self.gsheet_name} could not be accessed right now, the row "
                f"\n {row} \n"
            ...
            if backup_data:
                ...
                backup_filename = find_next_available_filename(
                    p.Path(self.BACKUP_FILENAME)
                )
                print(
                    f"Also pickling this to file {backup_filename}, which can be loaded"
    ...
    def verify_log_format(log_format):
        ...
        for col_name, col_type in log_format.items():
            if not isinstance(col_name, str) or not isinstance(col_type, type):
                print(
                    f"Column name & type combination of {col_name, col_type} is not valid."
            ...
            if col_type.__str__ is object.__str__:
                warn(
                    f"{col_name} is of type {col_type} which does not have an implemented"
    ...
    def verify_log_data(self, log_data):
        ...
        for col_name, col_value in log_data.items():
            if col_name not in self.log_format:
                print(
                    f"{col_name} was not found in log_format. Log data must consist only "
            ...
            if not isinstance(col_value, required_col_type):
                print(
                    f"Value of [{col_name}, {col_value}], is not of the required type "
        ...
        for col_name in self.log_format.keys():
            if col_name not in log_data:
                print(
                    f"{col_name} was found to be missing from log_data. Log data requires all"
