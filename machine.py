...
class Machine(object):
    ...
    def __init__(
        ...
        if scheduler_name.lower() not in SCHEDULERS:
            raise NotImplementedError(
                "Specified queue type is not currently supported."
            )
    ...
    def get_n_jobs(self, requested_walltime, safe_job_time_fl=False):
        ...
        if isinstance(requested_walltime, str):
            hrs, mins, secs = (
                int(quantity) for quantity in requested_walltime.split(":")
            )
    ...
    def check_nodes(self, cpus, nodes, allow_remainder_fl=False):
        ...
        if cpus_per_node_remainder > 0 and allow_remainder_fl is False:
            raise ValueError(
                f"Number of processors chosen does not divide equally between the nodes on {self.name}. \n"
                "There must be an equal number of processors used on each node. \n"
            )
        if cpus_per_node > self.max_cpus_per_node:
            raise ValueError(
                f"Number of processors requested does not fit onto {nodes} node(s) on {self.name}\n"
                f"You will need to increase the number of nodes requested. \n"
            )
        if cpus_per_node < self.max_cpus_per_node:
            print(
                "WARNING: Inefficient number of processors chosen - you won't be fully utilising every node. Your \n"
                "account may still be charged for all nodes occupied \n"
            )

        if nodes > self.max_nodes:
            raise ValueError(
                f"Number of processors requested would require more nodes ({nodes}) than the maximum \n"
                f"available on this machine ({self.max_nodes})\n"
            )
        elif nodes == self.max_nodes:
            print(
                "WARNING: Using maximum acceptable number of nodes on this machine. If you have any currently \n"
    ...
    def get_isolated_node_distribution(self, cpus, nodes):
        ...
        if (
            minimum_cpus > self.max_cpus_per_node
            or maximum_cpus > self.max_cpus_per_node
        ):
            raise ValueError(
                f"Number of processors required to isolate the first node does not fit onto {nodes} "
...
cumulus = Machine(
    "Cumulus", 32, 512, 16, None, "pbs", modules=["ifort/2017.0.098", "icc/2017.0.098"]
)
